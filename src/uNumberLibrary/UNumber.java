 ...
 * 
 * @version 1.03	Addition of sqrt method to calculate square root of UNumbers.
 * 
...
public class UNumber implements Comparable<UNumber> {
	 ...
	 */
	protected void normalize(UNumber that){
		if (that.d[0]==0){
	...
	private static int howManyDigitsMatch(UNumberWithGetters newGuess, UNumberWithGetters oldGuess, int maxMatchingDigits) {
		...
		// The characteristic is the same, so fetch the mantissas so we can compare them
						...
						int maxIterations = maxMatchingDigits;                 // Set the upper limit; 

						for (int ndx = 0; ndx < maxIterations; ndx++) {        // Loop through the digits as long as they match
	...
	/**********
	 * sqrt method is to calculate the square root using UNumbers by Newton Raphson Method
	 * 
	 * @return	square root of the UNumber
	 * 
	 * @param theValue  SquareRoot of the operand should be calculated
	 */
	
	public UNumber sqrt(double theValue) {
						...
						int numSigDigits = 25;									// This is for computing results using UNumber value decimal places

						UNumberWithGetters theValue2 = new UNumberWithGetters(theValue);	// We will compute the square root of this value
						...
						iteration = 0;														// This is used to count the number of iterations
						...
						do {
							...
							oldGuess2 = newGuess2; 										    // Save the old guess
							
							newGuess2 = new UNumberWithGetters(theValue2); 			     	// Compute the new guess
							
							newGuess2.div(oldGuess2);                                       // Compute according to Newton Raphson Method formula
							newGuess2.add(oldGuess2);
							newGuess2.div(two2);
							...
							/*
							 * the below if condition is to break some exceptions that cause the loop
							 * to iterate infinitely for example - 16, 121, 169 etc.
							 */
							...
							digitsMatch = howManyDigitsMatch(newGuess2, oldGuess2, numSigDigits);	// Determine how many digits match
						...
						/*
						 * below conditions are for singular and plural forms of word iteration
						 */
